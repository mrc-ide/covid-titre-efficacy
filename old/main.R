...
rho <- 0.8 # proportion of antibody response that is short-lived versus long-lived (not yet applied)
...
draw <- function(mu_ab = mu_ab, std = std, std10 = std10, ab_50 = ab_50, ab_50_severe, rho = rho, dr = dr, t = t, dr_long = dr_long, logk = logk){
  ...
  init_titre <- rnorm(1, mu_ab, )
  # antibody titre in natural log units
  nt <- NULL
  nt[1] <- init_titre
  for (i in 2:length(t)){
...
g1 <- ggplot(data = filter(r1, type == "Titre")) +
  ...
  labs(x = "time (days)", y = "neutralising antibody titre (log scale)") +
