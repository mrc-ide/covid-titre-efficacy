...
mu_ab_list <- data.frame(name = c("AstraZeneca", "Pfizer", "Moderna"),
                         mu_ab_d1 = c(1/59,13/94,((185+273)/2)/321),            
                         mu_ab_d2 = c(32/59,223/94,654/158)) %>%        #32/59,223/94,654/158    
...
#  dr_vec[t_d2:(t_d3+t_d2-1)] <- dr_vec_sub[t_d2:(t_d3+t_d2-1)]
  ...
  ef_transmission <- 1 / (1 + exp(-k * (log10(nt/12) - log10(ab_50))))
  ef_severe <- 0
  
  # output list containing titre and vaccine efficacy
  ret <- list(titre = nt, VE = ef_transmission, VE_severe = ef_severe)
  return(ret)
}
